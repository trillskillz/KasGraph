...
- **kasgraph-store** now has two schema slices: the initial covenant-lineage + POI + RPC audit tables, plus a `kasgraph_committed_block` + `kasgraph_reorg_audit` slice that powers committed-state unwind. The live `Store` exposes `record_committed_block` and `unwind_committed_blocks_for_subgraph` (single SQL transaction: ordered deletion of POI + audit + committed-block rows followed by a reorg-audit insert).
