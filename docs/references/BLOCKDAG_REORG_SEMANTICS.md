...
| Continuous wRPC subscription | Yes — `MultiRpcClient::spawn_continuous_subscription` runs forever with exponential backoff on transport errors, resets backoff on clean disconnect, cooperatively exits when the consumer drops the receiver (via `tokio::select!` on `sender.closed()`), and gives up after `SubscriptionBackoff::max_attempts`. Wired end-to-end into `kasgraph-node` behind `KASGRAPH_INGEST_MODE=continuous`: spawns the driver, consumes `ChainNotification`s from an `mpsc` channel, and calls the shared `apply_and_persist_notification` helper for each one. Supports bounded runs (`KASGRAPH_CONTINUOUS_MAX_MESSAGES`) for smoke tests and Ctrl-C for graceful shutdown. |
