...
| Continuous wRPC subscription | **RPC primitive landed; node-side wiring pending** — `MultiRpcClient::spawn_continuous_subscription` runs forever with exponential backoff on transport errors, resets backoff on clean disconnect, cooperatively exits when the consumer drops the receiver (via `tokio::select!` on `sender.closed()`), and gives up after `SubscriptionBackoff::max_attempts`. Wiring this into the node's persistence loop (replacing the current bootstrap pass) is the next jump. |
