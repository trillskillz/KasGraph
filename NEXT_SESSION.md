...
## Latest commit arc (2026-05-29 — SDK manifest validator + build-time contract gate)
...
- **Next honest, unblocked dev-time slice candidate:** wire `validateManifest` into `kasgraph codegen` too (it parses the same manifest at `cli/src/codegen.ts:95`), so codegen rejects a bad manifest with the same precise errors rather than producing confusing partial output. Pure/dev-time, no new infra.
