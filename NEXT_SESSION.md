...
## Latest commit arc (2026-05-29 — legacy KRC-20 inscription envelope parser)

New subsystem started, now that the covenant/spend/lineage track is complete (and its remaining gap — the native `operation` decoder — is blocked on OpenSilver script semantics). Legacy (Kasplex-era) KRC-20 is the unblocked alternative: its operations are **JSON inscriptions in the transaction payload field** (protocol-observable), not covenant scripts. Commit `ebed8bf` (`kasgraph-detectors`).
...
- **Pure core first** — no wire/store change. The envelope lives in the tx `payload` field, which the wire model (`IngestedBlock`) does **not** carry yet (it has `outputs` + `inputs` only).
- **Next slices (in order) to make legacy KRC-20 real end-to-end:** (1) add the per-transaction `payload` bytes to the `kasgraph-rpc` wire model (mirror how `inputs` were added — extract the tx `payload` hex from the wRPC tx shape) and thread through `BootstrapBlock`; (2) a **ledger acceptance state machine** applying the Kasplex rules (deploy first-writer-wins; mint `cumulative_minted + amt <= max && amt <= lim`; transfer `sender_balance >= amt`; burn decrements both) — sender = the tx's first input address, so this needs input→address resolution; (3) a dedicated `kasgraph_krc20_legacy_ledger` table keyed on `(tick, accepting_block_hash, seq)` with reverse-acceptance-order reorg unwind (the lineage-row model does NOT apply here — see `KRC20_KRC721_REFERENCE.md:54`). Needs Postgres to verify the ledger end-to-end; the parser and (likely) the pure state-transition function are unit-testable without it.
