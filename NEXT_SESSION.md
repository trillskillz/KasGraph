...
Autonomous work picked up by the next agent run. Phases 1–4 and 6 are substantially landed (269 tests green: 111 cargo + 158 TS). The **Phase 2.6 WASM mapping runtime is real** — `kasgraph-mapping` runs subgraph mappings deterministically on wasmtime (fuel-metered, NaN-canonicalized, fresh `Store` per block), with a concrete host/guest ABI. **Spend-semantic payload codegen is now landed too** — `CovenantSpent` handlers emit a typed `{ spend: CovenantSpend; state }` envelope. The remaining track that builds on the runtime: (a) the TS→WASM `kasgraph build` CLI command (AssemblyScript `asc` targeting the ABI), which also unblocks `deploy`/`status`/`logs`/`remove`. Other autonomous tracks (need network/Postgres to verify): live-node wRPC recovery validation, real-Postgres `sqlx::test` coverage, real OpenSilver compiled-byte sync.

## Latest commit arc (2026-05-28 — spend envelope on CovenantSpent payloads)
