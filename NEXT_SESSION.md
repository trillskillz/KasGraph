...
> **PROJECT STATUS (2026-05-30): the indexer is FEATURE-COMPLETE.** The full `kasgraph build → deploy → index → query` loop is multi-tenant, hot-reloadable, and verified end-to-end against real Postgres. Every self-contained logic track is done — detection (17/17 real fingerprints), operation decode + live `CovenantSpent` dispatch, POI over dispatched state, the GraphQL query layer (entity/spends/typed per-subgraph schemas + relations + first-class lineage DAG), the KIP-20 lineage fork model, the deploy pipeline (registry → typed routing → CLI → HTTP `--node` endpoint → wasm data-plane → node consume → multi-subgraph fan-out → dynamic reload). **Remaining is operational/infra only: `logs` streaming + auth for the hosted service** (both need a topology/identity steer). See the dated commit arcs below for the detail; the most recent is at the top. Default test baseline: `cargo test --workspace` green + `npx vitest run` 238 | 1 skipped (the skipped one is the DATABASE_URL-gated e2e), plus the Rust `integration-pg` + TS e2e suites green against the local Postgres container.
